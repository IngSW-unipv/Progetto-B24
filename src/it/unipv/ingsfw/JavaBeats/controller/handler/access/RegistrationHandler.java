...
import it.unipv.ingsfw.JavaBeats.exceptions.AccountAlreadyExistsException;
      ...
      public void handle(ActionEvent actionEvent){
        ...
        if(!Pattern.matches(nameRegex, gui.getName().getText()) || !Pattern.matches(nameRegex, gui.getSurname().getText())){
          ...
          gui.getErrorMessage().setText("");
                    ...
                    try {
                        //Check if the profile exists or handles the exception
          ...
          }catch(AccountAlreadyExistsException | UsernameAlreadyTakenException | AccountNotFoundException e){
