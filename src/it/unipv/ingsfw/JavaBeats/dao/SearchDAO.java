...
public class SearchDAO implements ISearchDAO{
  ...
  //PUBLIC METHODS:
  @Override
  public ArrayList<IJBResearchable> search(String field, JBProfile activeProfile) throws AccountNotFoundException{
    return search(field, activeProfile, null);
  }

  @Override
  public ArrayList<IJBResearchable> search(String field, JBProfile activeProfile, EJBENTITY mode) throws AccountNotFoundException{
  ...
  private ArrayList<Song> searchSongs(String field, JBProfile activeProfile) throws AccountNotFoundException{
    connection=DBManagerFactory.getInstance().getDBManager().startConnection(connection, schema);
    PreparedStatement st;
    ResultSet rs;
  ...
  private ArrayList<Episode> searchEpisodes(String field, JBProfile activeProfile) throws AccountNotFoundException{
    connection=DBManagerFactory.getInstance().getDBManager().startConnection(connection, schema);
    PreparedStatement st;
    ResultSet rs;
    ...
    try{
      ...
      while(rs.next()){
        tmp.add(new Episode(rs.getInt("id"), null, null, null));
      }
    ...
    AudioDAO aDAO=new AudioDAO();
    for(Episode episode: tmp){
      result.add(aDAO.getEpisode(episode, activeProfile));      //get complete info of episode
    }
  ...
  private ArrayList<Playlist> searchPlaylists(String field) throws AccountNotFoundException{
    connection=DBManagerFactory.getInstance().getDBManager().startConnection(connection, schema);
    PreparedStatement st;
    ResultSet rs;
