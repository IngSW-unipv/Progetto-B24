...
public class AudioDAO implements IAudioDAO{
  ...
  @Override
  public void updateIsFavorite(JBProfile activeProfile){
    connection=DBManagerFactory.getInstance().getDBManager().startConnection(connection, schema);
    PreparedStatement st;
  ...
  @Override
  public ArrayList<JBAudio> selectByPlaylist(Playlist playlist, JBProfile activeProfile){
    connection=DBManagerFactory.getInstance().getDBManager().startConnection(connection, schema);
    PreparedStatement st;
    ResultSet rs;
    ...
    for(JBAudio audio: result){
      audio=get(audio, activeProfile);
    }
  ...
  @Override
  public ArrayList<Song> selectByAlbum(Album album, JBProfile activeProfile){
    connection=DBManagerFactory.getInstance().getDBManager().startConnection(connection, schema);
    PreparedStatement st;
    ResultSet rs;
    ...
    try{
      ...
      while(rs.next()){
        resultSongs.add(new Song(rs.getInt("idSong"), null, null, null));
      }
    ...
    for(Song song: resultSongs){
      result.add(getSong(song, activeProfile));
    }
  ...
  @Override
  public ArrayList<Episode> selectByPodcast(Podcast podcast, JBProfile activeProfile){
    connection=DBManagerFactory.getInstance().getDBManager().startConnection(connection, schema);
    PreparedStatement st;
    ResultSet rs;
    ArrayList<Episode> result=new ArrayList<>();
    ArrayList<Episode> resultEpisodes=new ArrayList<>();
    ...
    try{
      ...
      while(rs.next()){
        resultEpisodes.add(new Episode(rs.getInt("idEpisode"), null, null, null));
      }
    ...
    for(Episode episode: resultEpisodes){
      result.add(getEpisode(episode, activeProfile));
    }
